...
| `interior_daylight_window.png` | 48×40 | single | **daytime twin** — high sun + bloom, pale blue sky, clouds, snow-capped ridge, treeline, sunlit sill. Identical geometry, so it's a straight sprite swap |
| `interior_window_cycle.png` | 768×40 | **16 frames** | the **day→night cycle** — the same window baked at 16 points around the clock. Slice Grid By Cell Count **16 × 1** (48×40). Frame 0 dawn · 6 noon · 12 sunset · 15 night |
...
- **Daytime swap:** assign `interior_daylight_window.png` to the same window renderer, then swap the
  lighting to match — ambient up to a neutral warm-white (RGB ≈ 190,195,185), the moonlight spot
  retargeted as a **sunlight** spot (warm white ~ (255,240,205)) at a steeper angle, and the shaft
  quad tinted warm. Dust motes read even better in a sun shaft; keep them. The sprite already bakes
  the sun bloom, the sunlit sill and the light bleeding round the curtain edges, so keep the in-engine
  sun from blowing the room out.

### Day → night cycle (`interior_window_cycle.png` + `WindowDayNightCycle.cs`)

Rather than snapping between the day and night sprites, drive the window through the 16-frame cycle
sheet from one normalised clock value — `0` dawn, `0.40` noon, `0.78` sunset, `1` deep night. Baked by
`windowcyclegen.js`; scrub it in `Window Cycle.dc.html`.

What moves across the 16 frames: the **sun** rises at the left pane, arcs up across both panes and
sets behind the right curtain, reddening as it drops; the **sky bands and horizon haze** grade dawn →
blue → gold → violet → night; **ridge, snow caps, treeline and field** relight and go dark; **clouds**
take the sun's colour; **stars** fade in past dusk; the **moon** lifts into exactly the position it
occupies in `interior_colddusk_window.png`, so the cycle closes on the sprite already in the game;
and the **sill warmth + curtain tint** track the light so the frame never looks pasted on.

`WindowDayNightCycle.cs` has two modes: **Stepped** (one renderer, snaps to the nearest frame — fine
at 16 frames) and **Crossfade** (two stacked renderers, alpha-blends frame N → N+1 — use it when the
window is large on screen). It also drives the room: `SampleLight(t)` returns the light colour for the
current hour from the same keyframes the art uses, so the shaft light, its angle and the floor shaft
quad follow the glass automatically — no second set of numbers to keep in sync.
