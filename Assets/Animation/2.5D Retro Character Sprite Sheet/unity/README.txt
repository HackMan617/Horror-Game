...
WHAT'S HERE
  ...
  character_master_long.png        Front-facing FEMALE / long-hair master sheet (mid-back hair).
  character_master_back_long.png   Back-facing FEMALE / long-hair master sheet.
...
THE IDEA
  One master sheet per facing PER BODY TYPE, recolored per character at spawn. You store only a
  CharacterLook (5 color ints + body type) per player — never a generated image.

  The long-hair (Female) sheets are painted in the SAME baked source colors as the
  Male sheets (hair base/shadow + the magenta eye sentinel), so every hair color
  option recolors them identically — no extra palette work.
...
IMPORT SETTINGS (important — do this for ALL FOUR .png files)
...
SETUP
  1. Drop all four .png and both .cs files into your project (e.g. Assets/Characters/).
  ...
  3. Assign masterFront = character_master, masterBack = character_master_back,
     masterFrontLong = character_master_long, masterBackLong = character_master_back_long.
  4. Set the `look` indices + `look.body` (or assign from your save data) and press Play.
     (Leave the Long fields empty to fall back to the Male sheets.)
...
CHANGING THE LOOK AT RUNTIME (creation menu / wardrobe)
  Set the indices and rebuild:
      ...
      characterAnimator.look.body  = BodyType.Female;   // long hair
...
SAVING PER PLAYER
  CharacterLook is [System.Serializable] — write the 5 ints + body type to PlayerPrefs,
  JSON, or your save system. That's the entire per-character footprint.
