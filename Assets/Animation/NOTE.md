...
## note_sign — one-off interactive prop

A much simpler piece: a scrap of paper reading "ON VACATION, WE'LL BE BACK NEVER." Not part of
the furniture atlas — its own sheet (`note_sign.png`, 176×80), one neutral palette, no nightmare
pass (it's a single-use read, not a room fixture that needs to rot).

```js
// [x, y, w, h, frames]
const NOTE = {
  far:  [ 0,  0, 16, 22, 2],   // seen across the room — pinned, writing illegible
  near: [ 0, 22, 88, 58, 2],   // the read/interacted view — actual text on the paper
};
```

Both states hold 2 idle frames (a slow paper droop) — swap `far` → `near` on interact; the text
itself doesn't animate. Live reference: `Note Sign.dc.html`.
